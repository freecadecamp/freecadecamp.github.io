...
(function() {
  ...
  function injectTranslateFAB() {
    ...
    function handleTranslationFadeIn() {
      const html = document.documentElement;
      
      // If we're not translating, do nothing
      if (!html.classList.contains('translating')) return;

      // Check if already translated
      if (html.classList.contains('translated-ltr') || html.classList.contains('translated-rtl')) {
        html.classList.remove('translating');
        return;
      }

      // Set up a MutationObserver to watch for Google Translate classes
      ...
      // Safety fallback: 1.5 seconds maximum wait time
      setTimeout(() => {
        html.classList.remove('translating');
        observer.disconnect();
      }, 1500);
    ...
    function doTranslate(code) {
      ...
      // Add translating class to trigger opacity: 0
      document.documentElement.classList.add('translating');
      handleTranslationFadeIn();
    ...
    // Check if we are currently translating on load and start observer
